...
本文维护 `@flyingcoding/dsh-magic-context@0.1.0-alpha.1` 的验收证据，范围为 `dsh_main` 已提交的轻量原生记忆实现，精确版本见下表。交付物为私有本地安装包，正式 profile 启用和 npm 发布属于独立动作。以下结论对指定夹具、版本和机器具有高置信度，适用限制列于文末。
...
机器可读摘要见[验收证据](evidence/2026-09-13-acceptance.json)。以下命令构成可复现验收路径。包内被忽略的 `.cache/` 回执保留准确安装包路径、原始测量、模型计数和 Session 日志位置，数据均为合成事实，不是用户历史对话。
