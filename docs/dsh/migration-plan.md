...
Execute one dependency-ordered stage at a time. The P0–P4 lightweight release is implemented and passes the acceptance recorded in the compatibility matrix. [DSH-only specialization](dsh-only-plan.md) is also complete; P5 remains optional and requires a separate need. Deliverables below belong to this fork; the [first-release baseline](README.md#p0) summarizes the accepted scope.
